...
- **🎙️ YouTube-like Chinese-first performance transfer.** Smart dubbing now persists one prosody contract for translation timing, pauses, speech act, and synthesis. A few verified bootstrap clips become per-speaker Chinese conditioning anchors; English source clips remain identity/performance input instead of conditioning every Chinese sentence. The former per-line source-clone route is still available as an explicit advanced policy.
