...
- **🇨🇳 Strict Chinese-output validation.** Latin terms explicitly present in the translated line remain allowed; any other Latin token is sent to repair. Clean preflight clips are promoted to per-speaker Chinese conditioning anchors so the rest of an F5 run no longer keeps prompting from English reference speech.
